...
def test_smooth_render_tracks_preserves_observed_keyframes(
    tmp_path,
) -> None:
    store = _store(tmp_path, source_fps=30, analysis_fps=10)
    profile = build_full_frame_profile(width=200, height=100)
...
def test_pchip_interpolation_preserves_keyframes(tmp_path) -> None:
    store = _store(tmp_path, source_fps=30, analysis_fps=10)
    profile = build_full_frame_profile(width=200, height=100)
    config = AppConfig.model_validate(
        {"render": {"smoothing": {"interpolation_method": "pchip"}}}
    )
    records = [
        _record(0, 0.0, [_track(1, 0, 0.0, BBox(x1=0, y1=10, x2=20, y2=30))]),
        _record(3, 0.1, [_track(1, 3, 0.1, BBox(x1=30, y1=10, x2=50, y2=30))]),
...
def test_pchip_interpolation_uses_curve_not_linear_for_accelerating_motion(
    tmp_path,
) -> None:
    store = _store(tmp_path, source_fps=30, analysis_fps=10)
    profile = build_full_frame_profile(width=200, height=100)
...
def test_pchip_interpolation_respects_max_gap_seconds(tmp_path) -> None:
    store = _store(tmp_path)
    profile = build_full_frame_profile(width=200, height=100)
    config = AppConfig.model_validate(
        {"render": {"smoothing": {"interpolation_method": "pchip"}}}
...
def test_pchip_interpolation_falls_back_to_linear_for_two_keyframes(tmp_path) -> None:
    store = _store(tmp_path, source_fps=30, analysis_fps=10)
    profile = build_full_frame_profile(width=200, height=100)
    config = AppConfig.model_validate(
        {
...
def test_pchip_interpolation_clamps_extreme_curve_to_linear_reference(
    tmp_path,
) -> None:
    store = _store(tmp_path, source_fps=30, analysis_fps=10)
...
def test_pchip_interpolation_does_not_extrapolate_after_track_ends(
    tmp_path,
) -> None:
    store = _store(tmp_path, source_fps=30, analysis_fps=10)
    profile = build_full_frame_profile(width=200, height=100)
