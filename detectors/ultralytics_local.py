...
class UltralyticsLocalDetector(Detector):
    ...
    def detect(self, image: np.ndarray) -> list[Detection]:
        ...
        allowed_names = {
            name.lower() for name in self.config.detector.allowed_class_names
        }
        ...
        for box, confidence, class_id in zip(xyxy, confidences, class_ids, strict=True):
            ...
            if (
                self.config.detector.allowed_class_ids is not None
                and class_id not in self.config.detector.allowed_class_ids
            ):
                continue
