...
- Render smoothing generates `analysis/render_frames.jsonl` for annotation only. By default, observed tracker boxes are exact anchors and missing source frames are linearly interpolated between adjacent observations, so moving vehicles do not trail behind their real position. Set `render.smoothing.observed_box_smoothing: causal_average` to opt into legacy moving-average smoothing with `supervision.DetectionsSmoother`; `render.smoothing.history_length` only applies in that mode and can visibly lag behind moving objects. Interpolation is render-only and does not affect counts, crops, make/model classification, or raw tracks. Generated interpolation frames may carry display state such as `counted`, but count events remain anchored to raw analysis records. `render.smoothing.max_interpolation_gap_seconds` controls gap bridging; `null` uses a cadence-based default. No tail extrapolation is generated.
